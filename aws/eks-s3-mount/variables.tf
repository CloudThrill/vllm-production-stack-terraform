...
variable "bootstrap_model_to_s3" {
  description = "Bootstrap model to S3 from Hugging Face if missing"
  type        = bool
  default     = true
}

variable "huggingface_model_id" {
  description = "Hugging Face model ID to download for bootstrap"
  type        = string
  default     = "TinyLlama/TinyLlama-1.1B-Chat-v1.0"
}
#--------------------------------------------------------------
# S3 Mountpoint Storage for vLLM Models
#--------------------------------------------------------------


variable "enable_s3_model_storage" {
  description = "Enable S3-backed storage for vLLM models"
  type        = bool
  default     = false
}

variable "enable_s3_csi_driver" {
  description = "Install AWS Mountpoint S3 CSI driver"
  type        = bool
  default     = false
}

variable "s3_bucket" {
  description = "S3 bucket name (example: vllm)"
  type        = string
  default     = "vllm-repo" # Keep as is, but user must ensure global uniqueness might neeed to change it
}

variable "s3_models_prefix" {
  description = "Prefix inside the bucket where models live (example: models)"
  type        = string
  default     = "models"
}
...
variable "s3_csi_driver_version" {
  description = "AWS Mountpoint S3 CSI driver Helm chart version"
  type        = string
  default     = "1.10.0"
}
################################################################################
