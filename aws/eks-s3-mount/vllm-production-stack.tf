...
data "template_file" "vllm_values" {
  ...
  vars = {
    s3_tiny_model  = local.s3_models["tiny"]
    s3_llama_model = local.s3_models["llama"]
    s3_qwen_model  = local.s3_models["qwen"]

    bootstrap_dependency = var.enable_s3_model_storage ? join("", terraform_data.bootstrap_model_to_s3[*].id) : ""
  }
}
