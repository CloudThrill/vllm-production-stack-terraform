...
End-to-end deployment for the DeepSeek-V3.2 reasoning API (685B, FP8) runs in roughly **95 minutes** — most of it model load, not infrastructure.
...
| **Phase 3** | **GPU NodePool (2×)** | ~15 min | 2× H100 bare-metal nodes, once the CPU pool is stable. |
| **Phase 4** | **DeepSeek-V3.2 model** | ~59 min | 643GB FP8 weight pull + dual-node VRAM load (PP=2) + DeepGEMM/FlashMLA warmup. |
| **Total** | **Full Stack** | **~95 min** | From zero to a 16×H100-backed reasoning API. |
